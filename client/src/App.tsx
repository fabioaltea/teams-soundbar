...
import { Share24Regular, Checkmark24Filled } from '@fluentui/react-icons';
...
function App() {
  ...
  const [meetingUrl, setMeetingUrl] = useState<string>((new URLSearchParams(window.location.search)).get("meeting") || "");
    ...
    <FluentProvider theme={teamsLightTheme}>
      ...
      <div style={{ display: 'flex', flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10, marginTop: 20 }}>
        ...
        <div className="container">
          ...
          <button>
            {!copied ? (
              <Share24Regular onClick={() => {
                const currentUrl: string = window.location.href;
                navigator.clipboard.writeText(`${currentUrl}?meeting=${meetingUrl}`);
                setCopied(true);
              }} />
            ) : (
              <Checkmark24Filled 
